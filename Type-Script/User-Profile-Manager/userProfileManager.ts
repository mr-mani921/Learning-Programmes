...
type LazyUser = Partial<User>;

class UserProfile {
  private users: LazyUser[] = [];
  checkExistence<T extends UserKeys>(key: T, value: User[T]): number {
  ...
  ageValidator(age: LazyUser["age"]): boolean {
  ...
  createNewUserProfile(newUser: LazyUser): void {
    if (newUser.id !== undefined) {
      const index = this.checkExistence("id", newUser.id);

      if (index === -1) {
        if (
          this.emailValidator(<string>newUser.email) &&
  ...
  updateUser<T extends UserKeys>(
    founderKey: T,
    founderKeyValue: User[T],
    updaterKey: T,
    updaterKeyValue: User[T]
  ): void {
    if (updaterKey === "id") {
